...
def train_xgboost_cpu(df):
    ...
    print(f"Total Missing Values : {D.isnull().sum().sum()}")
    ...
    # ================= AFTER PREPROCESSING =================
    processed_rows = D.shape[0]
    processed_cols = D.shape[1]

    print("\n" + "="*50)
    print("AFTER PREPROCESSING")
    print("="*50)
    print(f"Rows    : {processed_rows}")
    print(f"Columns : {processed_cols}")
    print(f"Total Missing Values : {D.isnull().sum().sum()}")

    print("\nRows Removed :", original_rows - processed_rows)
    print("New Columns Added :", processed_cols - original_cols)
    print("Retention Percentage : {:.2f}%".format(
        processed_rows / original_rows * 100
    ))
    ...
    # print(f"Processed Dataset     : {processed_rows} rows × {processed_cols} columns")
    # print(f"Rows Removed          : {original_rows - processed_rows}")
    # print(f"Features Added        : {processed_cols - original_cols}")
    # print(f"Data Retained         : {processed_rows/original_rows*100:.2f}%")
    ...
    with open("preprocessing_summary.txt", "w") as f:
        ...
        f.write(f"Processed Columns: {processed_cols}\n")
        f.write(f"Rows Removed: {original_rows - processed_rows}\n")
        f.write(f"Features Added: {processed_cols - original_cols}\n")
        f.write(f"Retention: {(processed_rows/original_rows)*100:.2f}%\n")
